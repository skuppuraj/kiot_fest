...
export async function getStaticProps() {
  return {
    props: {
      initialEvents: ALL_EVENTS
    }
  };
}

export default function EventsCatalogPage({ initialEvents }) {
  const [selectedDept, setSelectedDept] = useState('ALL');
  const [searchQuery, setSearchQuery] = useState('');

  const departments = ['ALL', 'CSE', 'ECE', 'AI&DS', 'MECH'];

  const filteredEvents = (initialEvents || ALL_EVENTS).filter((e) => {
  ...
  return (
    <div className="min-h-screen bg-[#0a0f1d] text-slate-100 flex flex-col justify-between">
      <Head>
        <title>All Competitions & Workshops | KIOT FEST 2026</title>
        <meta name="description" content="Browse all college symposium competitions, hackathons, and technical workshops" />
      </Head>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1">
              Explore events across CSE, ECE, AI&DS, and Mechanical departments
            </p>
          </div>
        </div>
