...
export async function getStaticProps({ params }) {
  const event = ALL_EVENTS.find(e => e.id.toString() === params.id.toString()) || null;
        ...
        <div className="bg-slate-900/90 border border-slate-800 rounded-3xl p-6 sm:p-10 backdrop-blur-xl relative overflow-hidden shadow-2xl">
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="text-right">
              ...
              <span className="text-2xl font-black text-amber-400">
                {Number(event.fee || event.registration_fee || 0) === 0 ? 'FREE' : `₹${event.fee || event.registration_fee}`}
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-8 pt-8 border-t border-slate-800/80">
            <div className="bg-slate-950/60 p-4 rounded-2xl border border-slate-800/60">
              ...
              <span className="text-base font-extrabold text-white">{event.prize || 'Certificates & Medals'}</span>
            </div>
            ...
            <div className="bg-slate-950/60 p-4 rounded-2xl border border-slate-800/60">
              ...
              <span className="text-base font-bold text-white">KIOT Campus</span>
            </div>
          </div>
